...
const STATUS_TONE: Record<string, string> = {
  ...
  Blocked: "text-[#FF6B9D] bg-[#FF6B9D]/10",
};
...
export default function DevicesManagement() {
  ...
  const loadDevices = async () => {
    ...
    try {
      ...
      if (response.isFailure) throw new Error(response.error?.description || "Không tải được thiết bị");
      const data = response.value ?? [];
      setDevices(data);
      setSelectedId((current) => current ?? data[0]?.deviceId ?? null);
    } catch (error) {
      showError("Không tải được thiết bị", error instanceof Error ? error.message : "Lỗi không xác định");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openCreate = () => { setForm(emptyDevice()); setDrawerMode("create"); setActiveTab("create"); };
  const openEdit = () => { if (!selected) return; setForm(mapDeviceToForm(selected)); setDrawerMode("edit"); setActiveTab("edit"); };
  ...
  const deleteDevice = async () => {
    if (!selected) return;
    if (!window.confirm(`Xóa thiết bị ${selected.deviceId}?`)) return;
    setSaving(true);
    try {
  ...
  const issueToken = async () => {
    ...
    try {
      ...
      if (res.isFailure || !res.value) throw new Error(res.error?.description || "Phát token thất bại");
      setIssuedToken(res.value.token);
      showSuccess("Token mới đã tạo", `TokenId: ${res.value.tokenId}`);
    } catch (error) {
      showError("Phát token thất bại", error instanceof Error ? error.message : "Lỗi không xác định");
    }
  };
  ...
  const revokeToken = async () => {
    ...
    try {
      const res = await adminService.revokeDeviceToken(selected.deviceId, tokenId.trim());
      if (res.isFailure) throw new Error(res.error?.description || "Thu hồi token thất bại");
      showSuccess("Đã thu hồi token", tokenId.trim());
      setTokenId(""); setIssuedToken(null);
    } catch (error) {
      showError("Thu hồi token thất bại", error instanceof Error ? error.message : "Lỗi không xác định");
    }
  };
  ...
  return (
    <div className="space-y-7 text-[#1A1A2E]">
      <AdminPageHeader
        ...
        stats={[{ label: "tổng thiết bị", value: devices.length }, { label: "đang hoạt động", value: activeCount }]}
        actions={
          <>
      ...
      {loading ? <AdminLoadingSkeleton rows={6} /> : (
        <CrudWorkspaceTabs
          ...
          stats={{ total: devices.length, activeLabel: "Đang hoạt động", activeValue: activeCount }}
        >
          <div className="grid gap-8 xl:grid-cols-[1fr_380px]">
            {/* ── Device list table ── */}
            <div>
              <p className="mb-3 text-xs font-black uppercase tracking-wide text-[#9CA3AF]">Danh sách thiết bị</p>
              {devices.length === 0 ? (
                <p className="text-sm text-[#9CA3AF] py-6 text-center">Chưa có thiết bị nào.</p>
              ) : (
                <div className="divide-y divide-[#F0F2F8]">
                  {devices.map((device) => {
                    const isActive = device.deviceId === selectedId;
                    const tone = STATUS_TONE[device.status] ?? "text-[#9CA3AF] bg-[#F0F2F8]";
                    return (
                      <button
                        key={device.deviceId}
                        type="button"
                        onClick={() => { setSelectedId(device.deviceId); setActiveTab("detail"); }}
                      ...
                      >
                        <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl text-base ${tone}`}>
                          {isActive ? <MdCheck /> : <span className="text-xs font-black">{device.serialNumber.slice(-2)}</span>}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-black text-[#1A1A2E] truncate">{device.nickname || device.serialNumber}</p>
                          <p className="text-[11px] text-[#9CA3AF] truncate">{device.deviceId}</p>
                        </div>
                        <span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-black ${tone}`}>{device.status}</span>
                      </button>
                    );
                  })}
                </div>
            ...
            {/* ── Detail panel ── */}
            <div>
              <p className="mb-3 text-xs font-black uppercase tracking-wide text-[#9CA3AF]">Chi tiết thiết bị</p>
              {selected ? (
                <div className="space-y-5">
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-widest text-[#9CA3AF]">Serial</p>
                    <p className="mt-1 text-lg font-black text-[#17409A]">{selected.serialNumber}</p>
                    <p className="text-xs text-[#9CA3AF] mt-0.5">{selected.deviceId}</p>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button type="button" onClick={() => void deleteDevice()} disabled={saving}
                  ...
                  <div className="pt-4 border-t border-[#F0F2F8] space-y-3">
                    <p className="text-[10px] font-black uppercase tracking-widest text-[#9CA3AF]">Quản lý token</p>
                    <button type="button" onClick={() => void issueToken()}
                    ...
                    {issuedToken && (
                      <div className="rounded-xl bg-[#F4F7FF] p-3">
                        <p className="text-[10px] font-black text-[#9CA3AF] mb-1">Token</p>
                        <p className="text-xs break-all font-mono text-[#1A1A2E]">{issuedToken}</p>
                      </div>
                    )}
                    <div className="flex gap-2">
                      ...
                      <button type="button" onClick={() => void revokeToken()} disabled={!tokenId.trim()}
                ...
                <p className="text-sm text-[#9CA3AF] py-6 text-center">Chọn một thiết bị để xem chi tiết.</p>
              )}
            </div>
          </div>
        </CrudWorkspaceTabs>
      ...
      <CrudEditorDrawer open={drawerMode !== null} mode={drawerMode ?? "create"} title="Thiết bị" description="Thêm hoặc chỉnh sửa thông tin thiết bị SmartBear." onClose={() => setDrawerMode(null)}>
        <div className="space-y-5">
          <label className={labelCls}>Device ID
            <input value={form.deviceId} onChange={(e) => setForm((p) => ({ ...p, deviceId: e.target.value }))} className={fieldCls} />
          </label>
          <label className={labelCls}>Số serial
            <input value={form.serialNumber} onChange={(e) => setForm((p) => ({ ...p, serialNumber: e.target.value }))} className={fieldCls} />
          </label>
          <label className={labelCls}>Tên thiết bị
            <input value={form.nickname ?? ""} onChange={(e) => setForm((p) => ({ ...p, nickname: e.target.value }))} className={fieldCls} />
          </label>
          <label className={labelCls}>Trạng thái
            <input value={form.status} onChange={(e) => setForm((p) => ({ ...p, status: e.target.value }))} className={fieldCls} />
          </label>
          <button type="button" onClick={() => void saveDevice()} disabled={saving}
            ...
            {saving ? "Đang lưu..." : drawerMode === "edit" ? "Lưu thay đổi" : "Tạo thiết bị"}
          </button>
        </div>
      </CrudEditorDrawer>
